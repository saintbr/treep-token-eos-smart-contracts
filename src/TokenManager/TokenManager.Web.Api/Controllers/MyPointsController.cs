...
{
    [Route("api/v1/tokens/mypoints")]
    public class MyPointsController : TokenBaseController<MyPoints>
    {
        public MyPointsController(EosBlockchainService eosBlockchainService) : base(eosBlockchainService) { }

        [HttpPost("transfer")]
        public override async Task<IActionResult> TransferAsync([FromBody] TransferRequest request)
        {
            var result = await _eosBlockchainService.TransferAsync("euroexchange", tokenDefinition.Symbol, tokenDefinition.Decimals, request.FromAddress, request.ToAddress, request.Quantity, request.Unstaked, request.Memo).ConfigureAwait(false);
            if (result.Item1)
            {
